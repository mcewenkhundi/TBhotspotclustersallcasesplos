...
#Model imported from the primary analysis of TBhotspot repository 
...
dat_cnr_2019 <- select(dat_scale_ln, cluster_number, tb_year, total_notified, population) %>% 
  filter(tb_year == 2019) %>%
#changed total_confirmed to total_notified
  mutate(cnr_2019 = (total_notified/population)*100000) %>%
...
cluster_ests_95_prev31_notif25_rintc %>%
  ...
  geom_hline(aes(yintercept=1.39), linetype=5, colour="darkgrey") +
  ...
  coord_flip() +
  scale_y_log10(labels=c(0.25,0.5,1,1.4,2,5,10,20),
                breaks=c(0.25,0.5,1,1.4,2,5,10,20)) +
  theme_ggdist() +
...
ggsave(filename = here::here("figures/S4_Fig.tiff"), height = 10, dpi = 310)
```
...
tmap_save(
  ...
  filename = here::here("figures/S5_Fig.tiff"),
